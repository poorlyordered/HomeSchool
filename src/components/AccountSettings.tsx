...
export function AccountSettings({ user, onClose }: AccountSettingsProps) {
  ...
  const handleChangeEmail = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ 
        type: 'error', 
        text: error instanceof Error ? error.message : 'An error occurred while changing your email' 
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6">
          {message && (
            <div className={`mb-4 p-3 rounded-md flex items-start gap-2 ${message.type === 'success' ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
          ...
          {activeTab === 'security' && (
            <div className="space-y-8">
              <form onSubmit={handleChangePassword} className="space-y-4 border-b pb-8">
